...
export class TransformsAnalyzer {
    ...
    static sanitizeImportedNodes(
        scene: Scene,
        meshes: AbstractMesh[],
        tns: TransformNode[],
        options: SanitizeOptions = {}
    ) {
        const uid = new ShortUniqueId({ length: options.guidLength ?? 10 });
        const mk = () => uid.rnd();

        // répertoire des IDs déjà pris dans la scène
        const taken = new Set<string>();
        for (const n of scene.getNodes()) if ((n as any).id) taken.add((n as any).id);
        ...
        const all: Array<AbstractMesh | TransformNode> = [...meshes, ...tns];

        for (const n of all) {
            const any = n as any;

            // ID unique (GUID si demandé ou si collision)
            if (options.forceGuidAll || !any.id || taken.has(any.id)) {
                const base = options.forceGuidAll ? mk() : any.id;
                any.id = makeUnique(base);
            } else {
                taken.add(any.id);
            }
            ...
            if (!options.keepOriginalName) {
                if (!any.name) any.name = any.id;
            }

            // Pour les TransformNodes: type Babylon pur + garder type custom en metadata
            if (n instanceof TransformNode) {
                if (any.customType && any.customType !== "BABYLON.TransformNode") {
                    any.metadata = any.metadata || {};
                    any.metadata.lgmCustomType = any.customType;
                }
                any.customType = "BABYLON.TransformNode";
                delete any.plugins;
                delete any.physics;
                delete any.physicsImpostor;
                delete any.stencil;
            }
        }
    }
